"""
Extends generate_scientific_diagram.py's figure with a second section
depicting the interior active-learning / manual-correction loop built on
top of the base pipeline (stages A-F, unchanged, reused as-is): proposing
extra candidate regions the base mask misses, a browser paint app for
directly adding/correcting/erasing regions, active-learning-driven manual
labeling, and a calibrated model that folds accepted regions back into a
single corrected final result.

Reuses every visual primitive (SVG class, icons, card/document drawing,
gradients, shadows) from generate_scientific_diagram.py so the new section
reads as one consistent figure, not a bolted-on second style.

Usage
-----
    python3 generate_full_workflow_diagram.py [image_name_without_extension]
"""
import os
import subprocess
import sys
...
# Second act: violet -> crimson, echoing the paint tool's own red/cyan/magenta
# palette while staying visually downstream of the sunset progression above.
# Exactly 2 entries -- this section is now just (G) Manual Correction and
# (H) Machine Learning, matching how this is actually used day to day.
...
def legend_card(title, swatches, accent):
    """A dedicated small graphic for a color legend -- kept as its OWN
    appropriately-sized image (like stat_card) rather than baked onto a
    large photo crop, which shrank illegibly once downscaled to thumbnail
    size in an earlier version of this figure."""
    W, H = 520, 430
...
def stat_card(title, lines, accent):
    """A small hand-drawn 'figure' summarizing the calibration numbers as
    plain text -- kept as a raster image (not raw SVG) purely so it slots
    into draw_card's existing image-thumbnail contract unchanged."""
    W, H = 520, 430
    img = Image.new("RGB", (W, H), "#FFFFFF")
    draw = ImageDraw.Draw(img)
    draw.rectangle([0, 0, W - 1, H - 1], outline="#E1E8EB", width=2)
...
def icon_svg(kind, color, size=1.0):
    """New icon kinds for the active-learning section, falling back to the
    base pipeline's icon set for everything already defined there."""
    sw = 0.16 * size
    if kind == "layers":
        parts = []
...
def draw_card_v2(svg, x, y, w, h, stage, color, grad_id):
    """Same visual language as draw_card, but takes an explicit gradient id
    (the new section has its own 4-color palette, indexed independently of
    the base pipeline's 6) and uses the extended icon_svg above."""
...
def main():
    print(f"Building full-workflow SVG diagram for {IMAGE_NAME} ...")
    ...
    # ---------------------------------------------------- gather part-2 art
    # Exactly 2 new stages, matching how this is actually used: right after
    # detection (F), correct it by hand (paint app + harder-candidate review
    # sheets are both just ways of doing that); then train/apply the model.
    # (A separate "interior candidate proposal" stage was cut here -- it's a
    # real background mechanism, not something worked through as its own
    # step, so it's folded into stage H's own description instead of getting
    # its own box.)
    ...
    # Read the live model's own calibration rather than hardcoding a
    # snapshot -- confirmed these drift for real (e.g. a data-loss bug fix
    # earlier changed the pooled classifier's training set, which shifted
    # this calibration even though interior_fill's own label count didn't
    # change), so a baked-in number silently goes stale the next time
    # anyone retrains.
    import joblib
    sys.path.insert(0, os.path.join(IAL_ROOT, "code"))
    from train_interior_model import load_labeled_interior
    _bundle = joblib.load(os.path.join(IAL_ROOT, "models", "interior_model.joblib"))
    _rule = _bundle.get("interior_fill_rule")
    _labeled = load_labeled_interior()
    _if = _labeled[_labeled["CandidateType"] == "interior_fill"]
    _n_pos, _n_neg = int(_if["IsCrack"].sum()), int((~_if["IsCrack"]).sum())
    if _rule is not None:
        calib_card = stat_card("interior_fill calibration", [
            (f"{_n_pos} pos / {_n_neg} neg", "labeled training examples"),
            (f"{_rule['recall_on_known_positives']*100:.1f}% recall", "on known positives"),
            (f"{_rule['full_pool_accept_rate']*100:.1f}% accept", "of full candidate pool"),
        ], PART2_COLORS[1])
    else:
        calib_card = stat_card("interior_fill calibration", [
            (f"{_n_pos} pos / {_n_neg} neg", "labeled training examples"),
    ...
    final_overlay = Image.open(os.path.join(applied_dir, f"{IMAGE_NAME}_final_result.png")).convert("RGB")
    ...
    part2_stages = [
        dict(key="G", title="Manual Correction", icon="brush",
             ...
             thumbs=g_thumbs),
        dict(key="H", title="Machine Learning", icon="check",
             subtitle="LogReg + hybrid rule (leave-one-out validated) folds accepted regions into the final result",
    ...
    PART2_TITLE_H = 110
    PART2_GAP = 80
    PART2_CARD_W = CARD_W  # same size as the base pipeline's own cards -- this is 2 stages, not 4
    PART2_CARD_H = 460
    ...
    svg.raw(f'<text x="{W/2}" y="90" text-anchor="middle" font-family="Helvetica, Arial, sans-serif" '
            f'font-size="16" font-style="italic" fill="{SUBTEXT}">Full workflow: detection → manual correction → '
            f'machine learning, worked example: {esc(IMAGE_NAME)}</text>')
    ...
    # ==================================================== PART 2 SECTION
    # Exactly what actually happens after (F): correct it by hand, then
    # train/apply the model on those corrections. Two stages, not four.
    ...
    svg.raw(f'<text x="{W/2}" y="{part2_y0+18}" text-anchor="middle" font-family="Georgia, serif" '
            f'font-size="24" font-weight="700" fill="{PART2_COLORS[1]}">Manual Correction &amp; Retraining</text>')
    svg.raw(f'<text x="{W/2}" y="{part2_y0+42}" text-anchor="middle" font-family="Helvetica, Arial, sans-serif" '
            ...
            f'lets a human correct what it missed, then retrains on those corrections</text>')
    ...
    # input arrow from base-pipeline output (F) down into G
    ...
    # feedback loop: H's newly-labeled corrections retrain the model used
    # next time -- the actual iterative mechanism, drawn as a loop back to G.
    loop_y = hy1 + 55
    ...
    svg.raw(f'<text x="{(gx0+hx1)/2}" y="{loop_y+18}" text-anchor="middle" font-family="Helvetica, Arial, sans-serif" '
            f'font-size="12.5" font-style="italic" fill="{PART2_COLORS[1]}">newly labeled corrections retrain (H) '
            f'for the next round</text>')
    ...
    caption = (
        ...
        f"plus its manual-correction and retraining loop. Raw 16-bit micrographs are contrast-stretched and "
        f"auto-cropped (A), background-flattened (B), and segmented into candidate dark regions (C). A Frangi "
        f"vesselness filter and connected-component analysis extract an 8-feature vector per candidate (D), "
        f"which a logistic-regression classifier separates into cracks and artifacts; a minimum-spanning-tree "
        f"bridges fragments of the same physical crack (E), producing a binary mask, numbered overlay, and "
        f"measurements (F). Read-only against that result, a human can then directly add, correct, or erase "
        f"regions in a browser paint app using the same red=crack/cyan=artifact convention as (F) plus a magenta "
        f"eraser -- two touching red regions are always merged into one crack -- or work through review sheets "
        f"of harder candidates (notches along a crack's own boundary, corridors between bridged fragments, or "
        f"the wider gradient-fade interior a flattening filter can suppress) that the base mask's brightness "
        f"threshold missed (G). Those labels train a small classifier, calibrated with an extra distance/"
        f"brightness rule for the hardest candidate type and validated by leave-one-out testing against every "
        f"known negative rather than trusted on cross-validation alone; accepted regions are folded into the "
        f"crack mask, and the freshly labeled corrections retrain the loop for the next round (H). "
    ...
    svg_path = os.path.join(OUT_DIR, f"full_workflow_{IMAGE_NAME}.svg")
    with open(svg_path, "w") as f:
        f.write(svg.render())
    print(f"Saved SVG: {svg_path}")

    png_path = os.path.join(OUT_DIR, f"full_workflow_{IMAGE_NAME}.png")
