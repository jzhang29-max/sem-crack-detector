...
| threshold sensitivity | **done**: `experiments/threshold_sensitivity.py` sweeps 0.3–0.7 through both passes and reports both the magnitude and whether the ordering of conditions survives. The shipped operating point turned out to be the `0.5` fallback of `bundle.get("threshold", 0.5)` — a library default reached by omission, under every number here. |
