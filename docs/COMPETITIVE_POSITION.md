...
| threshold sensitivity | **done, and measured**: sweeping 0.3–0.7 through both passes on 9 frames moves crack count **1.28–1.41×** and area fraction only **1.02–1.17×**; within one specimen the count is **15× more sensitive** than the area. The condition ordering did **not** flip, which was the prediction, so the claim is "state your threshold" rather than "comparisons are wrong". The shipped operating point turned out to be the `0.5` fallback of `bundle.get("threshold", 0.5)` — a library default reached by omission, under every number here. |
