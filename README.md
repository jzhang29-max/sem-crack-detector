...
inside pip. The fifth runs the same install and suite on **macOS arm64**, because the two bugs
this workflow found first were platform-divergent in exactly this way — a PNG re-encode that was
byte-identical on macOS and different on Linux — and that divergence runs both directions.
